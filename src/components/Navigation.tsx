  ...
  return <nav className="w-full bg-white bg-opacity-95 backdrop-blur-sm fixed top-0 z-50 border-b shadow-lg">
      ...
      <div className={cn("md:hidden absolute top-full left-0 right-0 bg-white border-b transform transition-transform duration-300 ease-in-out shadow-lg", isOpen ? "translate-y-0" : "-translate-y-full")}>
        <div className="container mx-auto px-6 py-6 flex flex-col gap-6">
          <Link to="/" className="font-semibold text-lg text-gray-700 hover:text-orange-600 transition-colors py-2" onClick={() => setIsOpen(false)}>
            Home
          </Link>
          <Link to="/test" className="font-semibold text-lg text-gray-700 hover:text-orange-600 transition-colors py-2" onClick={() => setIsOpen(false)}>
            Placement Test
          </Link>
          <Link to="/test?practice=true" className="font-semibold text-lg text-gray-700 hover:text-orange-600 transition-colors py-2" onClick={() => setIsOpen(false)}>
            Speaking & Writing
          </Link>
          <Link to="/about" className="font-semibold text-lg text-gray-700 hover:text-orange-600 transition-colors py-2" onClick={() => setIsOpen(false)}>
            About Us
          </Link>
        </div>
      </div>
