      ...
      <div className="container mx-auto py-3 flex items-center justify-between px-6 md:px-12">
        <Link to="/" className="flex items-center gap-4">
          <img 
            ...
            className="h-16 md:h-20 w-auto transform hover:scale-105 transition-transform object-contain" 
        ...
        <div className="hidden md:flex items-center gap-8">
          {navItems.map((item) => (
            <Link 
              ...
              to={item.path} 
              className={cn(
                "font-semibold text-lg transition-colors px-3 py-2 rounded-lg relative",
        ...
        <div className="container mx-auto px-6 py-6 flex flex-col gap-4">
          {navItems.map((item) => (
            <Link 
              ...
              to={item.path} 
              className={cn(
                "font-semibold text-lg py-3 px-4 rounded-lg transition-colors min-h-[48px] flex items-center",
                "hover:text-pink-600 hover:bg-pink-50",
                isActive(item.path) 
